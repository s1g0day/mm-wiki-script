...
def nalyze_data(url, config, source, title):
    ...
    pattern_list = ["(?s)<div id=\"cnblogs_post_body\" class=\"blogpost-body blogpost-body-html\">(.*)<div id=\"blog_post_info_block\" role=\"contentinfo\">",
    "(?s)<div id=\"cnblogs_post_body\" class=\"blogpost-body cnblogs-markdown\">(.*)<div id=\"blog_post_info_block\" role=\"contentinfo\">"]
    for pattern in pattern_list:
        pattern = re.compile(pattern)
        match = pattern.findall(source)
        if match:
            content = match[0]
            content = re.findall(r'(?s)(.*)</div>\n<div class=\"clear\">', content)[0]
