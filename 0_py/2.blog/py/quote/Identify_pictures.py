...
def intern_save_img(md_path_name, img_data, filename):
...
def Pictures_processing(md_path_name, img_data, filename, count, base64_images, intern_imgs, local_imgs):
    # 处理base64图片
    if "data:image" in img_data:
        ...
        datas = intern_save_img(md_path_name, img_data, filename)
        if datas != None:
...
def md_img_readline(datas, img_data, md_path_name, filename, count, base64_images, intern_imgs, local_imgs):
    if ")](" in img_data:
        img_data_spl = img_data.split(")](", 1)[0]
        result_data, base64_images, intern_imgs, local_imgs = Pictures_processing(md_path_name, img_data_spl, filename, count, base64_images, intern_imgs, local_imgs)
        if result_data == img_data:
            datas = datas
        else:
            datas = result_data
    else:
        result_data, base64_images, intern_imgs, local_imgs = Pictures_processing(md_path_name, img_data, filename, count, base64_images, intern_imgs, local_imgs)
        if result_data == img_data:
            datas = datas
        else:
            datas = result_data
...
def images_save(md_path_name, files_name):
