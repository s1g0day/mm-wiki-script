# coding:utf-8
'''
@File: hander_random.py
@CreateTime: 2023/05/19 15:23:29
@UpdateTime: 2025/04/17 18:23:31
@Version: 0.0.5
@Description: 生成随机headers
@Note:
引用案例
...
def random_referer(url=""):
    ...
    if url:
        # 获取根域名
        data = f"http://www.{tldextract.extract(url).registered_domain}/{random.choice(pasta)}/{random.choice(arquivo)}.{random.choice(ext)}"
    else:
        data = f"http://www.{random.choice(dominio).lower()}.{random.choice(gTLD)}.{random.choice(locais)}/{random.choice(pasta)}/{random.choice(arquivo)}.{random.choice(ext)}"
    return data
# 生成随机useragent
def random_useragent():
    ...
    useragent = []
    for _ in range(3):
        header = f"{random.choice(agentBrowser)}/{str(random.randint(1, 20))}.{str(random.randint(1, 20))} ({random.choice(agentSistema)} {str(random.randint(1, 7))}.{str(random.randint(0, 9))}; {random.choice(locais)})"
        useragent.append(header)
    return " ".join(useragent)
...
def requests_headers(url=""):
    ...
    # 更新refer
    if url:
        # headers["Referer"] = random_referer(url)
        headers["Referer"] = url
    headers = headers.copy()  # 复制现有的请求头
    headers.update(generate_random_headers_X())  # 将随机生成的请求头追加到现有的请求头
