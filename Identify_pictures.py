...
@note:
    ...
    Dev:
        python3 Identify_pictures.py -path "E:\work\mm-wiki\wiki\1_file\3_http" -proxy 127.0.0.1:7890 -w
'''
...
def local_save_img(md_path_name, files_name, img_data, filename):
    ...
    '''
    # 规范化并验证图片路径
    normalized_img_path = local_img_path(md_path_name, files_name, img_data)
    
    if not normalized_img_path:
        logger.error(f"[-] {files_name} 本地图片无法访问: {img_data}")
        return f"\n\n![]({img_data})\n\n"

    # 获取图片扩展名
    ext = os.path.splitext(normalized_img_path)[1].lower()
    
    try:
        # 检查是否已在目标目录
        if check_img_exist(md_path_name, filename, normalized_img_path):
            return f"\n\n![]({normalized_img_path})\n\n"
        
        # 处理MP4文件
        if ext == '.mp4':
            logger.info(f"[*] {files_name} 包含本地mp4: {normalized_img_path}")
            # 复制MP4到临时位置
            temp_mp4_path = ensure_image_dir(md_path_name, filename, 'mp4')
            shutil.copy2(normalized_img_path, temp_mp4_path)
            return f"\n\n[{normalized_img_path}]({temp_mp4_path})\n\n"

        # 处理其他图片文件
        logger.info(f"[*] {files_name} 复制图片到目标目录: {normalized_img_path}")
        new_image_path = ensure_image_dir(md_path_name, filename, ext)
        # shutil模块复制文件
        shutil.copy2(normalized_img_path, new_image_path)
        # 覆盖datas的图片内容
        return f"\n\n![]({new_image_path})\n\n"
    except Exception as e:
...
def images_save(md_path_name, files_name, proxy, write_file=False):
    ...
    if file_ext != '.md':
        logger.info(f"[!] 跳过非MD文件: {files_name}")
        return None
    
    try:
        with open(files_name, 'r', encoding='utf-8', errors='ignore') as file_md_path:
            file_md_data = file_md_path.readlines()

            if not file_md_data:
                logger.info("\033[33m[+] 文件无内容\033[0m")
                return None
            
             # 预处理：分割连续图片
            file_md_list = []
            for line in file_md_data:
                if ")![" in line:
                    # 分割连续的图片标记
                    segments = line.replace(")![", ")\n![").split("\n")
                    file_md_list.extend(segments)
                else:
                    file_md_list.append(line)

            # 处理每一行
            count = 1
            img_pattern = re.compile(r"!\[(?P<des>.*?)\][(](?P<data>.*)[)]", re.DOTALL)
                    
            for line in file_md_list:
                # 查找图片标记
                md_img = img_pattern.search(line)
                ...
                if md_img:
                    ...
                    if img_data:
                        ...
                        count += 1
                # 将合成后的本地路径添加到列表
                md_data_images.append(line)

    except Exception as e:
        logger.error(f"[-] 处理文件时出错: {files_name}, 错误: {str(e)}")
        return None
    ...
    if not (base64_images or intern_imgs or local_imgs):
        logger.info("[+] 不存在图片,需人工确认")
    
    # 将图片修改的md写入文件
    if write_file:
        update_md(md_data_images, files_name)
        logger.info(f"[+] 已更新文件: {files_name}")
    return md_data_images
...
def process_directory(path, proxy, thread_count, write_file=False):
                ...
                if file.lower().endswith('.md'):
                    ...
                    if new_filename != filename:
                        ...
                        try:
                            ...
                            renamed_files.append((file, new_path))
    ...
    if all_files:
        ...
        # 输出重命名统计
        if renamed_files:
            logger.info(f"[+] 共重命名 {len(renamed_files)} 个文件:")
